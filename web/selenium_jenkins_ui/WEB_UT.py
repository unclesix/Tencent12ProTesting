...
class ISelenium(unittest.TestCase):
    # 读入配置文件,windows cmd打开就是“C:\Users\dell>”
    #ConfigParser：处理加载配置文件模块
    def get_config(self):
        config = configparser.ConfigParser()
        configFilePath = 'C:\\Users\\dell\\iselenium.ini'
        config.read(configFilePath)
        # config.read(os.path.join(os.environ['HOMEPATH'], 'iselenium.ini'))  #windows此方法无法根据“HOMEPATH环境变量读取的ini文件”
        return config
    ...
    def setUp(self):
        ...
        print(config)
        # 控制是否采用无界面形式运行自动化测试
        #using_headless=False 无环境变量 ，使用有界面方式运行
        try:
        ...
        #using_headless=True 无环境变量 ，使用无界面方式运行
        if using_headless is not None and using_headless.lower() == 'true':
            print('使用无界面方式运行')
            chrome_options.add_argument("--headless")
            #指定chrome.exe绝对路径。设置binary_location属性
            chrome_options.binary_location = r'D:\Python38\chromedriver.exe'
